...
const Sidebar = () => {
  ...
  return (
    <div className={`h-full  bg-slate-900 shadow-lg shadow-slate-950 z-50 transition-all relative duration-[0.1s] ease-in  ${openSidebar ? 'w-screen  md:w-64 xl:w-72 ':'w-0 md:w-28'}`}>

      <div className={`absolute  -right-4 ${openSidebar && 'right-1 rotate-180 md:-right-4 md:rotate-0'} mt-64    bg-slate-950 h-[10%] rounded-tr-2xl rounded-br-2xl cursor-pointer flex items-center justify-center group`} onClick={()=>setOpenSidebar(open=>!open)}>
      ...
      { tools.map(tool=>(
        <Link key={tool.id} href={tool.link} className={`flex gap-5 items-center
           ${pathname===tool.link ?'':'hover:opacity-80'} cursor-pointer group relative `} onClick={()=>{if(window.innerWidth <= 800){setOpenSidebar(false)}}}>
